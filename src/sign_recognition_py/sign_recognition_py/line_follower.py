...
class ImageSubscriber(Node):
    ...
    def process_image(self, img):
        ...
        if len(contours) > 0:
            ...
            # Dynamic speed control: the farther the line is from the center, 
            # the slower we go, to allow sharper turns. If the line is close to the center, we can speed up.

            # Calculate the error from the center of the image
            error = (cols / 2) - cx
            
            # Proportional control constant for angular velocity (tune this value for better performance)
            kp_angular = 0.005  
            
            # Calculate angular velocity based on the error
            msg.angular.z = error * kp_angular
            ...
            base_speed = 0.35
            msg.linear.x = max(0.08, base_speed - (abs(error) * 0.001))

            # If the line is lost, keep the last angular velocity to try to find it again
