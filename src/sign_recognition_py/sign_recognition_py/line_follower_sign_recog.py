...
class ImageSubscriber(Node):
    def __init__(self):
        ...
        self.curr_speed = 0.15

        self.last_sign = -1
        self.sign_count = 0
        self.confirmed_sign = -1

        self.sign_threshold = 3

        self.timer = 0
        self.stop_time = 10
        self.stopped = False
        self.disable_stop = False
        self.cornering = False


        # Set image size
        self.image_size_lf = 24
        self.image_size_sr = 48
        ...
        pkg_turtlebot3_mogi_py = get_package_share_directory('sign_recognition_py')
        model_path_lf = pkg_turtlebot3_mogi_py + "/network_model/model_line_follow.best.keras"
        model_path_sr = pkg_turtlebot3_mogi_py + "/network_model/model_sign_recognition.best.keras"

        print("Tensorflow version: %s" % tf.__version__)
        keras_version_str = str(keras_version)
        print("Keras version: %s" % keras_version_str)
        print("CNN model of line follow: %s" % model_path_lf)
        print("CNN model of sign recognition: %s" % model_path_sr)

        # Fetch the saved Keras version used to produce the file
        model_version = self.get_keras_version_from_keras_file(model_path_lf)
        ...
        # Finally load model:
        self.model_lf = load_model(model_path_lf, custom_objects=None, compile=True, safe_mode=True)
        self.model_lf.summary()

        self.model_sr = load_model(model_path_sr, custom_objects=None, compile=True, safe_mode=True)
        self.model_sr.summary()


        # Define a new model to extract intermediate layer outputs
        self.activation_model_lf_1 = tf.keras.models.Model(inputs=self.model_lf.inputs, outputs=self.model_lf.get_layer("conv2d_1").output)
        self.activation_model_lf_2 = tf.keras.models.Model(inputs=self.model_lf.inputs, outputs=self.model_lf.get_layer("activation_1").output)

        self.activation_model_sr_1 = tf.keras.models.Model(inputs=self.model_sr.inputs, outputs=self.model_sr.get_layer("conv2d_1").output)
        self.activation_model_sr_2 = tf.keras.models.Model(inputs=self.model_sr.inputs, outputs=self.model_sr.get_layer("activation_1").output)
    ...
    def process_image(self, img):
        ...
        image_lf = cv2.resize(img, (self.image_size_lf, self.image_size_lf))
        image_lf = img_to_array(image_lf)
        image_lf = np.array(image_lf, dtype="float") / 255.0

        image_sr = cv2.resize(img, (self.image_size_sr, self.image_size_sr))
        image_sr = img_to_array(image_sr)
        image_sr = np.array(image_sr, dtype="float") / 255.0

        image_lf = image_lf.reshape(-1, self.image_size_lf, self.image_size_lf, 3)
        image_sr = image_sr.reshape(-1, self.image_size_sr, self.image_size_sr, 3)
        
        with tf.device('/gpu:0'):
            prediction_lf = np.argmax(self.model_lf(image_lf, training=False))
            prediction_sr = int(np.argmax(self.model_sr(image_sr, training=False)))

        # Get the feature maps for the input image
        activations_1_lf = self.activation_model_lf_1.predict(image_lf)
        activations_2_lf = self.activation_model_lf_2.predict(image_lf)
        activations_1_sr = self.activation_model_sr_1.predict(image_sr)
        activations_2_sr = self.activation_model_sr_2.predict(image_sr)

        grid_image_1 = self.visualize_feature_maps(activations_1_sr, num_rows=5, num_cols=10, padding=1)
        grid_image_2 = self.visualize_feature_maps(activations_2_sr, num_rows=5, num_cols=10, padding=1)

        print("Prediction of line following %d, elapsed time %.3f" % (prediction_lf, time.time()-self.last_time))
        print("Prediction of sign recognition %d, elapsed time %.3f" % (prediction_sr, time.time()-self.last_time))
        self.last_time = time.time()

        if prediction_sr == self.last_sign:
            self.sign_count += 1
        else:
            self.last_sign = prediction_sr
            self.sign_count = 1

        if self.sign_count >= self.sign_threshold:
            self.confirmed_sign = prediction_sr


        if prediction_lf == 0: # Forward
            msg.angular.z = 0.0
            self.cornering = False
        elif prediction_lf == 1: # Right
            msg.angular.z = -0.3
            self.cornering = True
        elif prediction_lf == 2: # Left
            msg.angular.z = 0.3
            self.cornering = True
        else: # Nothing
            msg.angular.z = 0.2
        ...
        if not self.stopped:
            if self.confirmed_sign == 0:
                msg.linear.x = 0.05
                self.curr_speed = 0.05
            elif self.confirmed_sign == 1:
                msg.linear.x = 0.1
                self.curr_speed = 0.1
            elif self.confirmed_sign == 2:
                msg.linear.x = 0.15
                self.curr_speed = 0.15
            elif self.confirmed_sign == 3:
                msg.linear.x = 0.2
                self.curr_speed = 0.2
            elif self.confirmed_sign == 4:
        ...
        if self.disable_stop and self.confirmed_sign != 4:
            self.disable_stop = False
